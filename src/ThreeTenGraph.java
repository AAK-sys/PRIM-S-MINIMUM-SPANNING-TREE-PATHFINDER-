...
class ThreeTenGraph implements Graph<GraphNode,GraphEdge>, UndirectedGraph<GraphNode,GraphEdge> {
	...
	public ThreeTenGraph() {
		storage = new GraphComp[MAX_NUMBER_OF_NODES][MAX_NUMBER_OF_NODES];
	}
    ...
    public Collection<GraphEdge> getEdges() {
		...
		ArrayList<GraphEdge> info =  new ArrayList<>();
    for(int i = 0; i<MAX_NUMBER_OF_NODES; i++){
      for(int j = 0; j<MAX_NUMBER_OF_NODES; j++){
        if(storage[i][j]!=null && i!=j && !info.contains(storage[i][j])){info.add( (GraphEdge) storage[i][j]);}
      }
    }
    ...
    public Collection<GraphNode> getVertices() {
		...
		ArrayList<GraphNode> info = new ArrayList<>();

    for(int i = 0; i<MAX_NUMBER_OF_NODES; i++){
      if(storage[i][i]!=null){info.add( (GraphNode) storage[i][i]);}
    }
		//Hint: this method returns a Collection, look at the imports for
		//what collections you could return here.
    ...
    pos = vertex.getId();
    if(storage[pos][pos]!=null && vertex.equals(storage[pos][pos])){return true;}
    ...
    if(vertex==null || !containsVertex(vertex)){return null;}
		//O(n) amortized where n is the max number of vertices in the graph
    ArrayList<GraphNode> info = new ArrayList<>();
		ArrayList<GraphNode> temp =(ArrayList<GraphNode>) getVertices();

    for(int i = 0; i<temp.size();i++){
      if(findEdge(vertex, temp.get(i)) != null){info.add(temp.get(i));}
    }
    ...
    for(int i = 0; i<MAX_NUMBER_OF_NODES; i++){
      if(storage[pos][i]!=null && pos!=i){info.add((GraphEdge)storage[pos][i]);}
    }
    ...
    public Pair<GraphNode> getEndpoints(GraphEdge edge) {
		//O(n^2) where n is the max number of vertices in the graph
		Pair<GraphNode> holder = null;
    for(int i=0; i<MAX_NUMBER_OF_NODES; i++){
      for(int j=0; j<MAX_NUMBER_OF_NODES; j++){
        if(storage[i][j]!=null && storage[i][j].equals(edge) && i!=j){holder = new Pair(storage[i][i],storage[j][j]);return holder;}
        }
      }
		return null;
	}
    ...
    if(storage[x][y]!=null){holder = (GraphEdge) storage[y][x];}
    ...
    for(int i = 0; i<MAX_NUMBER_OF_NODES; i++){
      if(storage[pos][i]!=null && edge.equals(storage[pos][i])){return true;}
    }
    ...
    if(storage[vertex.getId()][vertex.getId()]!=null){return false;}

    storage[vertex.getId()][vertex.getId()]=vertex;
    ...
    public boolean removeEdge(GraphEdge edge) {
		...
		for(int i =0; i< MAX_NUMBER_OF_NODES; i++){
      for(int j = 0; j<MAX_NUMBER_OF_NODES; j++){
        if(storage[i][j]!=null && edge.equals(storage[i][j])){
     ...
     * @return true if the removal is successful, false otherwise
     */
    public boolean removeVertex(GraphNode vertex) { //still stuff to do.
		//O(n) where n is the max number of vertices in the graph
    if(!containsVertex(vertex) || vertex == null){return false;}
    int pos = vertex.getId();
    storage[pos][pos]=null;
		for(int i = 0; i<MAX_NUMBER_OF_NODES; i++){
      if(storage[pos][i]!=null){storage[i][pos]=null;storage[pos][i]=null;}
    }
    ...
    if(curr == null){
      DFT(str, smallestNode(visited), visited, restart);
    }else{
      DFT(str, curr, visited, restart);
    }
    if(restart==true){
      
      while(smallestNode(visited)!=null){
		...
		return str.toString(); //change later.
	}
  ...
  private boolean isdone(boolean[] done){
    ...
    if(count==getVertexCount()){return true;}
    return false;
  }

//==================================================================================================================
  private boolean DFT(StringBuilder str, GraphNode curr, boolean[] visited, boolean restart) {
    RecursionCheck.hasRecursion(); //DO NOT REMOVE THIS LINE

    if(isdone(visited)){return true;}
    
    visited[curr.getId()] = true;

    str.append(curr.getText() + " ");
    
    while(getnextneighbor(curr, visited)!=null){
      DFT(str, getnextneighbor(curr, visited), visited, restart);
    }
    
    if(!isdone(visited)){return false;}
    return false;
  }

//====================================================================================================================
  private GraphNode smallestNode(boolean[] done){
    ArrayList<GraphNode> temp = (ArrayList<GraphNode>) getVertices(); 
    GraphNode holder = null;

    for(int i=0; i<getVertexCount(); i++){
      if(!done[temp.get(i).getId()]){
        holder = temp.get(i);
        break;
      }
    }
    ...
    for(int i = 0; i<getVertexCount(); i++){
      if(holder.compareTo(temp.get(i))>0 && !done[temp.get(i).getId()]){holder = temp.get(i);}
    }
...
//=====================================================================================================================
  private GraphNode getnextneighbor(GraphNode node, boolean[] done){
    
    if(getNeighborCount(node)  == 0){return null;}
    
    GraphNode min = null;
    ArrayList<GraphNode> holder = (ArrayList<GraphNode>) getNeighbors(node);


    for(int i = 0; i<getNeighborCount(node);i++){
      if(!done[holder.get(i).getId()]){min = holder.get(i);break;}
    }
    for(int i = 0; i<getNeighborCount(node);i++){
      if(!done[holder.get(i).getId()] && min.compareTo(holder.get(i))>0){min = holder.get(i);}
    }
    ...
    if(graph.containsEdge(edges[2])){System.out.println("yay 0");}
    ...
    if(!graph.containsEdge(edges[7])){System.out.println("Yay 4");}

    if(!graph.containsVertex(new GraphNode(12))){System.out.println("Yay 5");}
