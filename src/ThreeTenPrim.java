...
class ThreeTenPrim implements ThreeTenAlg {
	...
	public GraphNode getMinimumIdNode() {
		...
		//YOUR_CODE_HERE
		ArrayList<GraphNode> holder = (ArrayList<GraphNode>) graph.getVertices();

		if(holder.size()!=0){
			minIdNode = holder.get(0);
		}

		for(int i = 0; i<holder.size(); i++){
			if(minIdNode.compareTo(holder.get(i))<0){minIdNode = holder.get(i);}
		}
	...
	public void start() {
		...
		for(GraphNode v : graph.getVertices()) {
			...
			if(n.graphNode.equals(minIdNode)){
				n.cost = n.graphNode.getId();
			}
